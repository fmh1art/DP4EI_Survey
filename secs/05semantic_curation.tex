...
preparation increasingly operates on subtask segments and state transitions.

Boundary signals include gripper or contact changes, object motion, end-effector
velocity, abrupt changes in learned representations, language-aligned task progress,
...
Foundation-model labeling provides task semantics.  Natural language Instruction
Labeling for Scalability (NILS) detects state changes and
uses pretrained models to produce language supervision without task-specific
training~\cite{blank2024scaling_2410_17772}.  Earlier, Data-driven Instruction
Augmentation for Language-conditioned Control (DIAL) propagated a small set of
human trajectory descriptions through a vision--language model (VLM) to augment
language supervision for a much larger robot dataset
~\cite{xiao2022robotic_2211_11736}.  Their strength is scalable relabeling, while
their risk is a correlated error chain in object detection, state-change
inference, locating events in time, and language generation.

A robust segmentation record should retain soft boundary confidence and the
signals that triggered it.  It should also preserve a hierarchy from episode to
phase, primitive, and transition.  Hard boundaries can then be sampled with tolerance during
training.  This representation supports three operations unavailable at episode
level.  A pipeline can discard only an idle or corrupted interval and attach
different language to individual subtasks.  It can also keep a failure prefix as
context while sending the recovery suffix to a recovery objective.
...
Auto-labeling should allow the model to abstain when it is uncertain.  Structured
fields for
objects, relations, state changes, actions, and outcomes should be generated
before free text.  Multiple views and temporal windows can expose inconsistent
labels.  Cross-field checks can test whether an asserted placement corresponds
to a changed object relation.  Confidence should be calibrated on a human audit
sampled across tasks, sources, and confidence levels.  Unresolved cases should
be routed to review.  Agreement among
several prompts of the same model is not independent evidence.  Stronger audits
use different model families and compare labels with measurable state or
execution where available.

Label utility must also be tested downstream.  A fluent detailed description may
increase superficial linguistic variety while fragmenting equivalent tasks into inconsistent
...
the training objective and stage.  A label hierarchy can preserve both coarse
sharing and task-specific detail.
...
as expert demonstrations is equally problematic.  The useful distinction is
structural.  A pipeline should label the cause and onset of a failure, separate
valid prefixes and recoveries, mark operator takeover and retry, and route each
part to a compatible objective.

Self-Supervised Data Filtering (SSDF) uses relations between expert and imperfect
data to select useful segments from failures.  Its simulation and Franka
experiments show that imperfect
...
Rules and simple statistics are inexpensive and interpretable.  Common signals
include range checks, idle ratio, path length, jerk, smoothness, action
saturation, success labels, and duplicate detection.  They are effective for
...
Representation and information measures support richer selection.  Embedding
distance can identify duplicates and coverage gaps.  Mutual information between
state and action favors demonstrations that are both informative and predictable.
The mutual-information curation study
...
state coverage.  This negative case is more informative than a universal
``less is more'' claim because estimator quality, granularity, and retained
coverage matter.  Smoothness-driven RINSE similarly supplies a preference for
...
Learned progress provides another decision signal.  SCIZOR learns a task-progress signal
...
Learner-aware methods ask how a training unit affects a particular learner or
target set.  Influence estimates approximate how changing the weight or
membership of a unit changes loss on that target.  DataMIL estimates data
...
~\cite{xu2026athena_2606_16208}.  These methods more directly approximate
Equation~\ref{eq:conditional-value}, but their estimates are tied to the chosen
target.  A narrow or biased validation set makes its preferences the
definition of quality.  Approximations used to avoid full retraining can be
unstable, and repeated
training, rollouts, or influence computation can dominate the saved training
cost.
...
rollout scale limit generality, but the methodological warning applies broadly.
A detector can achieve a high area under the receiver operating characteristic
curve (AUROC) by identifying a visible artifact while
...
A curation study should first establish whether the operation changed its
intended data property and what else changed at the chosen threshold.  It should
then test whether that intervention improves a trained policy under equal data
access and compute, reporting the target distribution and uncertainty.  The
first question diagnoses the mechanism.  Only the second supports a utility
claim.

\subsection{Mixtures, Sampling, and Role Routing}

Cross-source training adds a joint decision.  Proportions affect both
gradient scale and how often rare capabilities are seen.  Uniform sampling
overweights large sources.  Uniform-by-dataset can over-amplify tiny noisy
sources, while fixed hand weights become stale as the learner changes.  Re-Mix
learns mixture weights with an objective that protects the worst-performing data
group and reports
...
optimization target requires constraints and independent audits.

Language-model preparation offers transferable mechanisms, not transferable
quality definitions.  DoReMi learns domain weights from the relative loss of a
small proxy model, while Data Selection for Language Models via Importance
Resampling (DSIR)
...
$\rightarrow$ episode $\rightarrow$ segment, with minimum sampling shares that protect rare
...
globally good, a pipeline can send broad but weakly aligned video to
representation pretraining and aligned robot actions to behavior cloning, which
trains a policy to reproduce recorded actions.
Unsuccessful attempts with outcome labels can support value or recovery
learning, while uncertain synthetic episodes can train a verifier.
Stage-conditioned routing explains why industrial recipes
...
noise but also reject corrective behavior.  A learner-aware estimator can improve
near-term loss but narrow long-term transfer.  No single threshold optimizes
correctness, coverage and diversity, target alignment, and cost in every setting.
A robust preparation system should expose this trade-off and calibrate cheap proxies with
