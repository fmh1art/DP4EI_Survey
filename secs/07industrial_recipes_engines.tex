...
Industrial embodied models instead disclose a \emph{data recipe}, which is the combined choice of sources, transformations, labels, filters, sampling rules, training stages, and feedback loops.
These systems show which operations can be run at industrial scale, but several parts of a recipe usually change together.
A technical report may establish the performance of the complete model while leaving source weights, rejected-data statistics, labor, or intermediate checkpoints undisclosed.
...
Table~\ref{tab:industrial-recipes} reconstructs five public recipe families from primary reports.
We record only disclosed quantities and identify conclusions that are our synthesis.
The table compares the role of each source and operation instead of ranking reported performance numbers.
...
The scale is enabled by a chain that first retargets and smooths hand motion, removes hands and reconstructs the background, searches for a feasible robot base and an inverse kinematics solution, and depth-composites the robot.
...
It uses mobile-manipulator, static multi-environment, cross-embodiment laboratory, high-level, verbal-instruction, and web data for different purposes.
...
Instead of discarding every failure or suboptimal rollout, it annotates speed, quality, mistakes, strategy, and control mode and conditions the policy on the desired metadata ~\cite{intelligence20260_2604_15483}.
...
Reproducibility should be reported separately for raw data, transformation code, mixture, model, verifier, and evaluation.
...
The evaluation may record success, failure, intervention, uncertainty, safety, coverage, and subgroup outcomes.
...
At each round, it needs access to versioned data and model lineage, verifier calibration, observed coverage gaps, remaining budget, and safety risk.
It may then acquire, repair, label, segment, filter, route, mix, generate, train, audit, roll back, or stop.
