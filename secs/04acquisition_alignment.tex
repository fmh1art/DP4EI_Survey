...
includes source design when it intentionally controls compatibility or
...
the latter accepts heterogeneity and moves more alignment work after collection.
...
an expanding system~\cite{zhao2026axis_2607_21588}.  As a recent preprint, it is
...
robot access.  UMI reproduces relevant gripper geometry, recovers
...
egocentric human and robot demonstrations, whereas Hand-Object Interaction
Pretraining (HOP) reconstructs
three-dimensional hand--object interaction and retargets it into robot experience
...
and embodiment assumptions.  Simulation offers exact state and controlled
generation of rare cases but inherits asset, rendering, dynamics, and
contact mismatch.  Autonomous execution, as in AutoRT, produces records from the
current policy in the actual environment.  It directly exposes parts of the task
distribution the policy handles poorly but also concentrates data around its
existing competence
~\cite{ahn2024autort_2401_12963}.

No source dominates along correctness, diversity, scale, and target alignment.
It is more useful to ask which training role a source can support.  Web and human
video provide broad visual and task semantics.  Portable human demonstrations
provide motion and temporal structure, while multi-robot corpora expose shared
visual and action regularities.  Target-robot teleoperation calibrates control
and contact.
Successful demonstrations support task adaptation, and failed or recovery
executions support post-training and safety.  Routing by role
...
Episode count hides variation in task concepts, objects, backgrounds,
viewpoints, initial conditions, operators, embodiments, outcomes, and transition
density.  Repeating a narrow configuration can improve statistical precision
without expanding coverage.  Adding a new environment can expand coverage but
may also introduce a different action convention.  The acquisition target should
be a structured coverage record rather than a global quota.  A practical
collection system tracks cells such as $(\text{skill},\text{object},\text{scene},
...
unchanged across learners or collection protocols.  Its value lies in varying
object and environment diversity separately.

An active acquisition policy chooses what to collect next from current evidence.
It must balance expected training value, new coverage,
risk, and collection cost.  Uncertainty alone is unsafe because it can select
sensor corruption, infeasible tasks, or states from which the robot cannot
recover safely.  Candidate tasks also need executability and safety checks.  The
resulting record must be labeled well enough to support its intended training
role.  Most current systems still rely on hand-designed task proposals and
success filters rather than a calibrated estimate of marginal utility.
...
resolved before semantic scoring.  A minimum ingestion contract verifies file
readability, schema and version compatibility, units, tensor shapes, and required
fields.  It checks clocks, dropped frames, cross-stream synchronization, and
command-to-actuation delay.  Calibration, robot description, joint limits, reset
boundaries, and controller identity must also be present.  Sensor checks cover
blur, exposure, occlusion, saturation, missing values, and implausible jumps.
Action checks cover range, idle fraction, clipping, jerk, collisions, and
...
across vision, internal robot state, force and torque, audio, and touch is itself
a semantic
...
Structural alignment maps fields, modalities, missing degrees of freedom,
...
Spatial and control alignment specifies whether actions are joint
positions, velocities, absolute or delta end-effector poses, forces, or discrete
tokens.  It also records the reference frame, rotation convention, units,
controller gains, and whether a command is a target or an executed state.
Converting to a camera- or
...
Temporal alignment reconciles sensor rate, controller rate, observation
...
different conditional distribution.  Resampling should not introduce frequency
artifacts, and contact boundaries should not be averaged away.  Both command and
measured execution times should be retained when delays vary.

Behavioral alignment asks whether the same normalized action has the
...
encoder can incorporate kinematic graphs and control metadata.  Mapping an action
to another embodiment and back can test whether its meaning is preserved, while
a rollout can test whether the mapped action produces the intended state
transition.  Open X-Embodiment demonstrates the benefit of a common training
interface, but its heterogeneity also motivates learner-aware mixture methods
...
mirroring lateral actions and the words ``left'' and ``right'' creates confident label
...
contact.  An augmentation is valid only when the observation, action, and task
description preserve the same state transition.  Image plausibility alone is
insufficient.

Preparation artifacts also need a data contract that records source and license,
collection context, robot and controller, coordinate frames, clocks,
transformations, labels, confidence, routing decisions, relationships between
training and test records, and version hashes.  Without this contract, later influence
