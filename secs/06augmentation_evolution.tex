...
remains coupled to an executable state transition and a valid task.  We
...
Observation-level augmentation changes lighting, texture, background, camera
noise, crop, or viewpoint while intending to preserve state and action.
...
updating its camera pose or visibility metadata creates a mislabeled action
observation.
...
audit question, namely whether pixels outside the edited region, geometry, action
labels, and task success still describe the same transition.

Trajectory-level transformation perturbs initial states, object poses, timing,
or motion and updates actions through geometry, replay, or optimization.
It preserves more action structure than video editing but depends on accurate
object frames and on the task remaining valid after transformation.  Mirroring,
retiming, and spatial warping are
valid only when every coupled observation, action, and task field is transformed
consistently, as discussed in Section~\ref{sec:acquisition}.

Segment recomposition decomposes demonstrations into object-relative skills and
instantiates or stitches them in new scenes.  MimicGen is a representative
...
physically small yet leave the robot in a state from which the next primitive
cannot succeed.

Simulation-based task generation creates assets, scenes, goals, rewards, and
expert trajectories.  Its strength is exact machine-readable state and scalable
failure generation.  Its weaknesses are generator bias, solver artifacts, and
mismatch between simulation and reality.  Variation should be measured in task
factors that change behavior and contact, not only in visual texture.
...
asset library, reward generator, and planner's combined range of possibilities.  Task count alone
is not evidence of behavioral coverage.

Video and world-model generation predicts demonstrations or alternative
futures conditioned on language, actions, images, or geometry.  A world model
in this context is a model that predicts how an observed scene may evolve.
Xiaomi-Robotics-U0 targets embodied synthesis with multiple views and geometric constraints
...
rollout can hide inconsistent three-dimensional geometry, unreachable motion, or incorrect
contact.  Generated pixels do not automatically supply calibrated robot actions.
Recovering actions from pixels is a separate estimation problem whose
uncertainty must be stored.

\subsection{The Bottleneck Is Layered Validation}

Generation quality is sometimes evaluated with visual fidelity or a
vision--language score.  These tests can reject some defects but cannot establish
that a trajectory is executable or useful for training.  The
...
kinematic and physical checks.

\input{tables/validation_stack}

The layers fail in different ways.  A semantic verifier can accept a visually
...
models from different families, geometric and physical constraints, and real
...
recovery policy.  Routing should reflect the type of evidence available.  Valid
action supervision can enter policy training.  Semantically valid video with
uncertain actions is better suited to representation learning, while physically
invalid samples can serve as negative examples for a verifier.  Ambiguous
high-value cases should go to simulation or human review.  Evaluation should
report how many samples pass the checks required for their training role per
unit cost, not only raw generation throughput.

\subsection{Generative Diversity Can Be Illusory}

Counting prompts or rendered scenes overstates coverage when samples inherit the
same generator biases.  Diversity should be measured at several levels:
...
are more relevant to control than image embeddings alone, an intuition
...
miss new failure modes.  Periodic controlled retraining and rollout audits are
needed to recalibrate generation priorities.  Generation should target coverage
gaps and difficult but valid cases.  Independently calibrated verifiers should
then search for semantic and physical violations instead of rewarding output that
merely resembles the generator's training distribution.
