...
\subsection{From Static Pipelines to Feedback Controllers}

A static pipeline computes $\traindata=\pipeline(\rawdata)$ once.  A data-evolution
engine updates its preparation policy after observing a model:
\begin{align}
\theta_t &= \operatorname{Train}(\pipeline_t(\mathcal{D}_{\leq t})),\\
E_t &= \operatorname{Evaluate}(\theta_t;\tau),\\
(\pipeline_{t+1},q_{t+1}) &= \operatorname{Update}(\pipeline_t,E_t),
\end{align}
where $E_t$ includes successes, failures, interventions, uncertainty, safety
events, and subgroup performance; $q_{t+1}$ requests new collection, generation,
relabeling, or reweighting.  The defining property is not simply repeated
training, but causal use of feedback to change future data decisions.

RoboCat demonstrates a generalist agent that generates practice data and
fine-tunes on new tasks~\cite{bousmalis2023robocat_2306_11706}.  AutoRT combines
foundation-model task proposals, safety checks, and robot fleets to orchestrate
large-scale autonomous experience~\cite{ahn2024autort_2401_12963}.  Demo-SCORE
uses online policy outcomes to curate offline demonstrations
~\cite{chen2025curating_2503_03707}.  These systems mark a progression from
human-authored rules, through pretrained-model semantic decisions, to policy and
deployment feedback.  They still leave the attribution problem open: if
performance changes after data, model, objective, and task proposal are all
updated, which data decision was responsible?
DEED explicitly closes part of this loop by using deployment experience to
evolve the demonstration set~\cite{sis2026closing_2607_20345}.  Its framing
strengthens the case for treating preparation as a controller, while also making
versioned interventions and held-out audits essential for separating useful
feedback from self-reinforcing selection.

Generalist VLA recipes reinforce the lifecycle view.  RT-1 and RT-2 establish
large-scale real-world control and web-knowledge transfer
~\cite{brohan2022rt_2212_06817,brohan2023rt_2307_15818}; OpenVLA and Octo expose
open generalist training baselines~\cite{kim2024openvla_2406_09246,team2024octo_2405_12213};
and the $\pi$ family reports broad pretraining followed by task- and
embodiment-oriented adaptation
~\cite{black20240_2410_24164,intelligence20250_2504_16054,intelligence20260_2604_15483}.
These papers primarily contribute models, so their data recipes are evidence of
engineering practice rather than controlled comparisons of preparation
operators.  A survey must not attribute an end-to-end gain to curation when model
capacity, objectives, robot hours, or post-training also changed.

\subsection{Failure Modes of Self-Evolution}

Closed loops introduce risks absent from offline cleaning.

\begin{itemize}[leftmargin=*]
    \item \textbf{Selection collapse:} the policy succeeds where it already has
    support, so success-only collection narrows the distribution and hides weak
    regions.
    \item \textbf{Verifier drift:} a learned judge shares the policy's blind spots
    or becomes stale as behavior changes.
    \item \textbf{Error amplification:} generated or pseudo-labeled errors enter
    training, increase confidence, and are then accepted more readily.
    \item \textbf{Reward or simulator exploitation:} automatic collection targets
    outcomes that satisfy a proxy without solving the intended physical task.
    \item \textbf{Catastrophic mixture change:} aggressively emphasizing current
    failures erodes broad capabilities or rare safety behavior.
    \item \textbf{Unsafe exploration:} value-of-information objectives propose
    states outside validated recovery and intervention capacity.
\end{itemize}

Mitigations follow control-system principles: an immutable audited anchor set;
minimum mixture floors; independent and periodically refreshed verifiers;
shadow evaluation before data promotion; canary training; uncertainty-aware
abstention; versioned lineage; bounded update rates; and rollback of both model
and data policy.  The engine should retain structured failures rather than only
successful self-generated examples.  Otherwise it removes the signal needed to
detect its own distribution collapse.
