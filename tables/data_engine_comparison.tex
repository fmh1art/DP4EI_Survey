...
& New-task practice outcomes, foundation-model task proposals, physical
...
collection.
& Safety rules and human or environment constraints gate collection.  The loop
does not autonomously attribute gains among proposer, data, objective, and
model changes. \\
\midrule
$\pi^{*}_{0.6}$/RECAP
~\cite{intelligence20250_2511_14759}
& Terminal success and failure labels, time-to-outcome values, autonomous trials,
and optional human interventions.
& Train a language-conditioned value function on accumulated experience and
train the VLA to favor actions with higher predicted value.  Collection and
offline updates then repeat.
& Policy and value models are restarted from the audited pretrained checkpoint at each
...
& One human seed bootstraps imitation learning.  Reinforcement learning then
adjusts the imitation policy through exploration, and successful
...
the simulation model define the reachable distribution.  It does not discover
unanticipated failures in the real world. \\
\midrule
SOP
~\cite{pan2026sop_2601_03044}
& Fresh failures and interventions produced by the current policy, together with
recent online and offline training losses.
& A distributed actor fleet streams complete episodes to one learner.  A
task-balanced sampler mixes static and online buffers, and human-intervention or
...
& A value model predicts a distribution of possible returns from both offline
and online experience.  Its gradients update the action-generation module, and
the shared policy is repeatedly redeployed.
& A fixed offline reference, two value estimates combined conservatively, mixed
replay, and freezing the pretrained vision--language part of the policy during
online updates reduce instability.  Four-hour studies do not yet
establish long-term verifier stability or stability of the data distribution. \\
\midrule
Robot-Powered Data Flywheels
~\cite{grannen2025robot_2511_19647}
& Deployment novelty and errors, with environmental or catalog signals that can
...
\midrule
DEED (Data-Efficient Post-Training and Experience-Driven Learning)
...
& Makes demonstration evolution explicit, but controlled comparisons before
model replacement, independent verifiers, and rollback remain necessary when several loop
